...
# ── Mode C: 2026-only leave-one-date-out CV with ALL features ────────────────
...
print(f'    Mode C (2026 all-feat CV): AUC={mode_c_auc:.4f}  LL={mode_c_ll:.4f}')

if mode_a_probs is not None and mode_a_auc > mode_c_auc:
    best_mode  = 'A'
    val_probs  = mode_a_probs       # for pick selection
    eval_probs = mode_a_probs       # for metrics (already out-of-sample)
    model      = model_a
    scaler     = scaler_a
    active_features = CORE_FEATURE_NAMES
    print(f'  → Using Mode A (historical + adjustment)')
else:
    best_mode  = 'C'
    val_probs  = mode_c_full_probs  # for pick selection (full model on all 2026)
    eval_probs = cv_probs_c         # for metrics (unbiased CV)
    model      = model_c
    scaler     = scaler_c
    active_features = ALL_FEATURE_NAMES
    print(f'  → Using Mode C (2026 all-features)')

# ── 5. Model diagnostics ──────────────────────────────────────────────────────
print()
print('='*68)
print(f'  MODEL COEFFICIENTS  (Mode {best_mode})')
print('='*68)
coefs = model.coef_[0]
...
for i in order:
    ...
    print(f"  {active_features[i]:<16} {coefs[i]:>8.4f}  {abs(coefs[i]):>8.4f}  {direction}")
...
if best_mode == 'A':
    model_artifact['adj_feature_names'] = ADJ_FEATURE_NAMES
