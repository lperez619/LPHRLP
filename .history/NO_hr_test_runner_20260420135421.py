...
print(f'  {sum(len(v) for v in game_slices.values())} total game slots')
...
for ci, cfg in enumerate(ALL_TUNE_CONFIGS, 1):
    ...
    hr_prob = 1.0 - np.exp(-exp_hr)

    # Aggregate per game and score
    date_acc = []
    date_picks_map = {}
    for date_str in TEST_DATES:
        day_game_preds = []
        for (pk, start, end, had_hr, home, away) in game_slices[date_str]:
            p_no_hr = float(np.prod(1.0 - hr_prob[start:end]))
            day_game_preds.append((p_no_hr, pk, had_hr, home, away))

        day_game_preds.sort(key=lambda x: -x[0])
        picks = day_game_preds[:cfg['games_to_pick']]
        date_picks_map[date_str] = picks

        if not picks:
            date_acc.append(0.0)
        else:
            n_correct = sum(1 for p in picks if not p[2])
            date_acc.append(n_correct / len(picks))
    ...
    grid_results.append({
        ...
        'date_picks': date_picks_map,
    })

    if ci % 10000 == 0 or ci == n_configs:
        elapsed = time.time() - t_start
        rate = ci / elapsed if elapsed > 0 else 0
        best_so_far = max(r['avg_prec'] for r in grid_results)
        print(f'  {ci:>9}/{n_configs} configs  '
              f'{rate:,.0f}/s  best={best_so_far:.1%}  '
