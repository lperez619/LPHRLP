...
CORE_FEATURE_NAMES = [
    ...
    'log_prob',      # bookmaker implied home run probability (implied odds)
    'odds_x_power',  # synergistic interaction: Odds × Raw Power
    'odds_x_park',   # synergistic interaction: Odds × Stadium Factor
    'odds_divergence',# delta: How much do Vegas Odds disagree with Batter History
]
...
def row_to_core_features(row):
    ...
    log_platoon = _safe_log(platoon)
    log_power = _safe_log(brl_ratio * ev_ratio * hh_f)
    log_prob = _safe_log(b_odds)
    
    odds_x_power = log_prob * log_power
    odds_x_park = log_prob * log_park
    odds_divergence = log_prob - log_rate
...
def build_training_data(year, bvp_career=None, weather_splits=None):
    ...
    cache_key = f'train_full_v4_{year}_odds'
    ...
    for game_pk, box in boxscores.items():
        ...
        for side, is_home, opp_key in [('away', False, 'home_pitcher_id'),
                                       ('home', True, 'away_pitcher_id')]:
            ...
            for lineup_idx, batter in enumerate(lineup):
                ...
                row = _precompute_row(
                    ...
                    batter_zones=b_zones, pitcher_zones=p_zones, day_odds=load_day_odds(g_date_str)
                )
                if row is not None:
        ...
        for side, opp_id, is_home in [
            ...
            for lineup_idx, bid in enumerate(lineup):
                ...
                row = _precompute_row(
                    ...
                    batter_zones_global, pitcher_zones_global, day_odds=load_day_odds(date_str)
                )
                if row is not None:
                    lpos = 0.5  # Ignore batting order position
