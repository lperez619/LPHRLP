...
def build_rolling_cache():
    ...
    for year in [2024, 2025]:
        boxscores = load_cache(f'boxscores_{year}')
        if boxscores:
            print(f"Found {len(boxscores)} boxscores for {year}")
            for pk, box in boxscores.items():
                for side in ('home', 'away'):
                    for b in box.get(side, []):
                        all_batters.add(b['batter_id'])

        # Pitchers can actually be found in `train_stats_2024` and `train_stats_2025`
        stats = load_cache(f'train_stats_{year}')
        if stats and 'pit_st' in stats:
            for pid in stats['pit_st'].keys():
                all_pitchers.add(pid)
