...
# ── 2a-patch. Backfill recent form for cached dates that have empty data ──────
for date_str in TEST_DATES:
    dc = date_cache[date_str]
    # Re-fetch if empty OR if data was fetched with old broken API
    rb = dc.get('recent_bat', {})
    pr = dc.get('pitcher_recent', {})
    need_refetch = (not rb and not pr)
    if need_refetch:
...
best_ew_prec = 0.0
best_ew = 0.75
for ew in [0.50, 0.55, 0.60, 0.65, 0.70, 0.75, 0.80, 0.85, 0.90, 0.95, 1.00]:
...
print(f'  Top-10 precision: C={prec_c:.1%}  D={prec_d:.1%}  E={prec_e:.1%}  A={prec_a:.1%}')
