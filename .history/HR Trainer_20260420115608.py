...
CORE_FEATURE_NAMES = [
    ...
    'vuln_x_park',   # interaction: log_vuln × log_park
]
...
# Monotonic constraints for GBM: 1=must increase HR prob, 0=unconstrained
# Forces the model to respect baseball logic (e.g. better power → more HRs)
GBM_MONOTONIC_CST = [
    # ── core features (13) ──
    1,   # log_rate:     higher batter HR rate → more HRs
    1,   # log_vuln:     more vulnerable pitcher → more HRs
    1,   # log_park:     HR-friendly park → more HRs
    1,   # log_platoon:  platoon advantage → more HRs
    1,   # log_power:    barrel/EV/hard-hit → more HRs
    1,   # log_bt:       bat speed/blast → more HRs
    0,   # norm_rv:      arsenal matchup (can be negative)
    0,   # has_norm_rv:  indicator
    1,   # log_ha:       home/away factor → more HRs
    1,   # log_dn_bat:   day/night factor → more HRs
    0,   # rate_x_vuln:  interaction (leave free)
    0,   # rate_x_park:  interaction (leave free)
    0,   # vuln_x_park:  interaction (leave free)
    # ── adj features (9) ──
    1,   # log_wx:        better weather → more HRs
    0,   # log_st:        spring training (noisy)
    1,   # log_bvp:       BvP advantage → more HRs
    1,   # log_wx_hist:   weather history advantage → more HRs
    1,   # log_zone_match: zone matchup → more HRs
    1,   # log_form:      hot streak → more HRs
    1,   # log_pit_rf:    pitcher giving up HRs → more HRs
    1,   # log_rhr:       recent HR count → hot
    0,   # lineup_pos:    non-monotonic (3/4/5 best)
    # ── hist_prior (1) ──
    1,   # hist_prior:    higher prior → more HRs
]
...
def row_to_core_features(row):
    ...
    return np.array([
        ...
        log_vuln * log_park,    # interaction: vuln x park
    ], dtype=np.float32)
...
for date_str in TEST_DATES:
    ...
    m_gbm = HistGradientBoostingClassifier(
        ...
        monotonic_cst=GBM_MONOTONIC_CST,
...
def _top10_precision(probs, y, date_idx, game_idx):
    """Compute top-10 global precision across all dates."""
    hits = 0; total = 0
...
for ew in np.arange(0.30, 1.01, 0.01):
    ...
    _prec = _top10_precision(_ep, val_y, val_date_idx, val_game_idx)
...
# ── Mode F: Meta-learner stacking (LogReg on LR + GBM OOF predictions) ───────
from sklearn.linear_model import LogisticRegression
meta_X_oof = np.column_stack([cv_probs_c, cv_probs_d_cal])
meta_model = LogisticRegressionCV(
    Cs=np.logspace(-2, 2, 10), cv=5, penalty='l2',
    scoring='neg_log_loss', max_iter=2000, random_state=42,
)
meta_model.fit(meta_X_oof, val_y)
# OOF predictions via leave-one-date-out (fair comparison with Modes C/D/E)
...
meta_X_full = np.column_stack([mode_c_full_probs, mode_d_full_probs_cal])
mode_f_full_probs = meta_model.predict_proba(meta_X_full)[:, 1]
print(f'\n  [Mode F] Meta-learner (LR on [LogReg, GBM]):')
...
# Pick mode with best top-10 precision (not AUC)
prec_c = _top10_precision(cv_probs_c, val_y, val_date_idx, val_game_idx)
prec_d = _top10_precision(cv_probs_d, val_y, val_date_idx, val_game_idx)
prec_e = _top10_precision(cv_probs_e, val_y, val_date_idx, val_game_idx)
prec_f = _top10_precision(cv_probs_f, val_y, val_date_idx, val_game_idx)
prec_a = _top10_precision(mode_a_probs, val_y, val_date_idx, val_game_idx) if mode_a_probs is not None else 0.0
print(f'  Top-10 precision: C={prec_c:.1%}  D={prec_d:.1%}  E={prec_e:.1%}  F={prec_f:.1%}  A={prec_a:.1%}')
...
mode_labels = {
    ...
    'F': 'Meta-learner (LR + GBM stacking)',
}
...
_pwr = val_core_X[:, 4]
pick_probs = pick_probs + 1e-8 * (_pwr - _pwr.min())
