...
SPRING_CACHE = os.path.join(os.path.expanduser('~'), 'OneDrive', 'Desktop',
                             'hr_prediction', 'spring_2026_cache.json')
...
print('  Power data (barrel/exit velo/hard-hit)...'); barrel = fetch_statcast_power(PITCHER_SEASON)
print(f'  {len(barrel)} players')
print('  Bat tracking...');  bat_t  = fetch_bat_tracking(PITCHER_SEASON)
print('  Batter arsenal...'); bvp = fetch_pitch_arsenal('batter', PITCHER_SEASON)
print(f'  {len(bvp)} batters')
print('  Pitcher arsenal...'); pars = fetch_pitch_arsenal('pitcher', PITCHER_SEASON)
...
for date_str in TEST_DATES:
    ...
    print('  Career BvP...')
    bvp_career = fetch_bvp_career(all_bat, all_pit)
    print(f'  {len(bvp_career)} batters with BvP data')
