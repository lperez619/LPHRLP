    ...
    for side, opp_id, is_home in [
        ...
        for lineup_idx, bid in enumerate(lineup):
            ...
            if row is not None:
                ...
                adj_feat[-1] = lpos
                idx = len(rows)
                is_eligible = bid not in prev_hr_hitters
                rows.append({
...
if _dropped:
    ...
    for bid, team, lpos, reason in _dropped:
        print(f'    ID {bid:>8}  {team:<5}  pos {lpos}  -- {reason}')
...
# Display picks
print()
print(f"  {'#':>3}  {'Player':<24} {'Matchup':<30} {'Pos':>3} {'P(HR)':>7}  {'Prev HR?':>8}")
print(f"  {'-' * 80}")
for rank, (idx, prob) in enumerate(picks, 1):
    r = rows[idx]
    matchup = f"{r['away_team']} @ {r['home_team']}"
    prev = 'YES' if r['bid'] in prev_hr_hitters else ''
    print(f"  {rank:>3}  {r['name']:<24} {matchup:<30} {r['lineup_pos']:>3} {prob:>6.1%}  {prev:>8}")
...
for g in games:
    ...
    print(f"\n  {away} @ {home}  ({ts})")
    print(f"  {'#':>3}  {'Player':<24} {'P(HR)':>7}  {'Pos':>3}  {'Elig':>4}")
    print(f"  {'-' * 50}")
    for rank, i in enumerate(ranked[:12], 1):
        r = rows[i]
        elig_mark = 'yes' if r['eligible'] else 'SKIP'
        print(f"  {rank:>3}  {r['name']:<24} {r['prob']:>6.1%}  {r['lineup_pos']:>3}  {elig_mark:>4}")
