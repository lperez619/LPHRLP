...
FEATURE_NAMES = [
    ...
    'log_brl',       # barrel rate ratio vs league
    'log_ev',        # exit velo ratio vs league
    'log_hh',        # hard-hit factor
    ...
    'has_form',      # indicator: recent form data available
]
...
def row_to_features(row):
    """Convert a _precompute_row 25-tuple to a float32 feature vector."""
    (bid, pk, away_team, home_team,
     ...
     pit_rf, ha_f, raw_bvp,
     name, wx_hist_f, dn_bat_f) = row

    # Recent form: ratio of recent HR/PA to season HR/PA
    has_form_val = 0.0
    log_form_val = 0.0
    if r_pa >= 15 and season_hr > 0 and season_pa > 0:
        sr = season_hr / season_pa
        rr = (r_hr / r_pa) if r_pa > 0 else 0.0
        form_ratio = rr / max(sr, 1e-8)
        log_form_val = _safe_log(max(form_ratio, 0.01))
        has_form_val = 1.0
...
def build_training_data(year):
    ...
    cache_key = f'train_features_{year}'
    ...
    for game_pk, box in boxscores.items():
        ...
        for side, opp_key, is_home in [
            ...
            for batter in box.get(side, []):
                ...
                if row is not None:
                    X_rows.append(row_to_features(row))
    ...
    X = np.vstack(X_rows).astype(np.float32) if X_rows else np.empty((0, len(FEATURE_NAMES)), dtype=np.float32)
...
# ── 3a. Build 2026 validation feature matrix ──────────────────────────────────
print('\n  Building 2026 feature matrix...')
val_X_rows = []
val_y_rows = []
...
val_eligible = []      # bool per row — False if prev-day HR
...
for date_str in TEST_DATES:
    ...
    for row in precomputed_rows[date_str]:
        bid = row[0]; pk = row[1]
        val_X_rows.append(row_to_features(row))
        ...
        val_eligible.append(bid not in prev_hr)
...
val_X = np.vstack(val_X_rows).astype(np.float32)
...
if train_X is not None and len(train_y) > 100:
    # ── Mode A: train on historical, validate on 2026 ─────────────────────────
    print(f'\n  Mode: TRAIN on {"+".join(str(y) for y in TRAIN_YEARS)} '
          f'→ VALIDATE on 2026')
    scaler = StandardScaler()
    X_tr = scaler.fit_transform(train_X)
    X_vl = scaler.transform(val_X)

    model = LogisticRegressionCV(
        Cs=np.logspace(-3, 2, 20), cv=5, penalty='l2',
    ...
    # Also do leave-one-date-out on 2026 for unbiased per-date evaluation
    cv_probs = np.zeros(n_val, dtype=np.float64)
    for date_str in TEST_DATES:
        test_idx = np.array(val_date_idx[date_str])
        train_idx = np.array([i for i in range(n_val) if i not in set(test_idx)])
        sc = StandardScaler()
        Xtr_cv = sc.fit_transform(val_X[train_idx])
        Xte_cv = sc.transform(val_X[test_idx])
        m = LogisticRegressionCV(
            Cs=np.logspace(-3, 2, 10), cv=3, penalty='l2',
            scoring='neg_log_loss', max_iter=2000, random_state=42,
        )
        m.fit(Xtr_cv, val_y[train_idx])
        cv_probs[test_idx] = m.predict_proba(Xte_cv)[:, 1]
    cv_ll  = log_loss(val_y, cv_probs)
    cv_auc = roc_auc_score(val_y, cv_probs)
    print(f'  Leave-one-date-out CV — log-loss: {cv_ll:.4f}  AUC: {cv_auc:.4f}')

else:
    # ── Mode B: leave-one-date-out CV on 2026 only ────────────────────────────
    print('\n  Mode: LEAVE-ONE-DATE-OUT CV on 2026 (no historical data)')
    cv_probs = np.zeros(n_val, dtype=np.float64)
    for date_str in TEST_DATES:
        test_idx  = np.array(val_date_idx[date_str])
        train_idx = np.array([i for i in range(n_val) if i not in set(test_idx)])
        if len(train_idx) < 50 or val_y[train_idx].sum() < 2:
            cv_probs[test_idx] = n_hr / n_val   # fallback: base rate
            continue
        sc = StandardScaler()
        Xtr = sc.fit_transform(val_X[train_idx])
        Xte = sc.transform(val_X[test_idx])
        m = LogisticRegressionCV(
            Cs=np.logspace(-3, 2, 15), cv=3, penalty='l2',
            scoring='neg_log_loss', max_iter=2000, random_state=42,
        )
        m.fit(Xtr, val_y[train_idx])
        cv_probs[test_idx] = m.predict_proba(Xte)[:, 1]

    val_probs = cv_probs   # in CV-only mode, use CV predictions for everything
    # Fit final model on ALL 2026 data for coefficient reporting
    scaler = StandardScaler()
    X_all = scaler.fit_transform(val_X)
    model = LogisticRegressionCV(
        Cs=np.logspace(-3, 2, 15), cv=5, penalty='l2',
        scoring='neg_log_loss', max_iter=2000, random_state=42,
    )
    model.fit(X_all, val_y)
    print(f'  Best C = {model.C_[0]:.4f}')
    cv_ll  = log_loss(val_y, cv_probs)
    cv_auc = roc_auc_score(val_y, cv_probs) if val_y.sum() > 0 else 0.0
    print(f'  Leave-one-date-out CV — log-loss: {cv_ll:.4f}  AUC: {cv_auc:.4f}')
...
for i in order:
    ...
    print(f"  {FEATURE_NAMES[i]:<16} {coefs[i]:>8.4f}  {abs(coefs[i]):>8.4f}  {direction}")
print(f"  {'intercept':<16} {intercept:>8.4f}")

# ── 6. Evaluation metrics ─────────────────────────────────────────────────────
print()
print('='*68)
print('  EVALUATION METRICS (2026 validation)')
print('='*68)

# Use cv_probs for unbiased metrics
eval_probs = cv_probs if 'cv_probs' in dir() and cv_probs is not None else val_probs
...
# Calibration check: predicted vs actual in probability bins
print()
...
# ── 7. Pick selection grid (picks_skip × picks_take) ──────────────────────────
# Use the full model's probabilities for pick selection
pick_probs = val_probs   # from the trained model

print()
print('='*68)
print(f'  PICK SELECTION GRID  (skip × take, {len(TEST_DATES)} dates)')
print('='*68)
...
# ── 8. Per-date results (best pick config) ────────────────────────────────────
print()
print('='*68)
print(f'  PER-DATE RESULTS  (skip={best_skip}, take={best_take})')
print('='*68)
...
for date_str in TEST_DATES:
    ...
    for pk, row_indices in val_game_idx[date_str].items():
        ...
        for idx, prob in elig[best_skip:best_skip + best_take]:
            ...
            day_picks.append({
                ...
                'hr_prob': round(prob, 4), 'game_pk': m['game_pk'],
    ...
    # Per-date log-loss (all rows, not just picks)
    didx = val_date_idx[date_str]
    if didx:
        d_ll = log_loss(val_y[didx], eval_probs[didx])
    else:
        d_ll = 0.0
...
print(f'  Per-game picks — {last_date} (skip={best_skip}, take={best_take}):')
...
print(f'\n  Model saved to hr_logreg_model.pkl')
