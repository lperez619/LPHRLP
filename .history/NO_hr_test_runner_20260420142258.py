...
for date_str in TEST_DATES:
    ...
    if cached is not None:
        # Backfill recent form data if missing from old cache
        if not cached.get('recent_bat'):
            print(f'\n  --- {date_str} --- (cached, backfilling recent form)')
            all_bat_c = cached.get('all_bat', set())
            all_pit_c = {g[k] for g in cached['games'] for k in ('home_pitcher_id','away_pitcher_id') if g[k]}
            cached['recent_bat'] = fetch_recent_hitting(all_bat_c, date_str)
            cached['pitcher_recent'] = fetch_pitcher_recent(all_pit_c, date_str)
            print(f'  Recent form: {len(cached["recent_bat"])} batters, {len(cached["pitcher_recent"])} pitchers')
...
for date_str in TEST_DATES:
    dc = date_cache[date_str]
    for g in dc['games']:
        # home_lineup_ids / away_lineup_ids are the actual starting lineups
        # (only populated when the API had lineup data)
...
print('  Pre-computing batter-game rows (starters only)...')
pregame_rows = {}
for date_str in TEST_DATES:
    dc   = date_cache[date_str]
    rows = []
    for g in dc['games']:
        ...
        for side, opp_id, is_home in [
            ...
            for bid in info[side]:
                if bid in starters_2026:
                    rows.append((bid, opp_id, info['home_team'], pk,
                                  info['time_slot'], is_home,
                                  info['away_team']))
    pregame_rows[date_str] = rows

# ── Pre-compute config-invariant factors into arrays ──────────────────────────
import numpy as np

print('  Pre-computing factor arrays for vectorized grid search...')

# Build flat list of pre-computed row data + game structure
flat_base      = []   # config-invariant product per row
flat_raw_vuln  = []   # uncapped pitcher vulnerability
flat_brl_ratio = []   # brl / league_brl (for ** pwr_brl_exp)
flat_ev_ratio  = []   # ev / league_ev (for ** pwr_ev_exp)
flat_wrv       = []   # weighted run value per weighted usage
flat_wu        = []   # total weighted usage (>=0.40 means active)
flat_r_pa      = []   # recent PA
flat_r_hr      = []   # recent HR
flat_srate     = []   # season HR rate
flat_shr       = []   # season HR count
flat_raw_bvp   = []   # raw BvP factor (before weight blending)

# Game structure: for each date, list of (game_pk, start_idx, end_idx, had_hr,
#                                         home_team, away_team)
game_slices = {}   # {date_str: [(pk, start, end, had_hr, home, away), ...]}

row_idx = 0
for date_str in TEST_DATES:
    dc = date_cache[date_str]
    batter_stats = date_batter_stats[date_str]
    splits = dc.get('splits_25', splits_2025_global)
    game_slices[date_str] = []
    cur_game_pk = None
    cur_game_start = row_idx

    for (bid, opp_id, home_team, pk, time_slot, is_home, away_team) in pregame_rows[date_str]:
        b = batter_stats.get(bid, {})
        ...
        # Start new game slice?
        if pk != cur_game_pk:
            if cur_game_pk is not None and row_idx > cur_game_start:
                bm = dc['batter_map'].get(cur_game_pk, {})
                all_bids = set(bm.get('home', [])) | set(bm.get('away', []))
                had_hr = any(b2 in dc['actual'] for b2 in all_bids)
                game_slices[date_str].append((cur_game_pk, cur_game_start,
                                              row_idx, had_hr,
                                              cur_home, cur_away))
            cur_game_pk = pk
            cur_game_start = row_idx
        ...
        # Pitcher vulnerability (raw, uncapped)
        p = dc['pit_stats'].get(opp_id, {})
        ph = 'R'
        raw_vuln = 1.0
        if p and p.get('stat'):
            ps = p['stat']
        ...
        # Pitch matchup intermediates
        bp_data = bvp.get(bid, {})
        pp_data = pars.get(opp_id, {})
        wrv_per_wu = 0.0
        wu_val = 0.0
        if bp_data and pp_data:
            wrv = 0.0
            wu_acc = 0.0
            for p_code, p_data in pp_data.items():
                p_grp = PITCH_GROUPS.get(p_code, p_code)
                usage = p_data.get('usage', 0)
                b_rv = bp_data.get(p_code, {}).get('rv100')
                if b_rv is None:
                    for b_code, b_data in bp_data.items():
                        if PITCH_GROUPS.get(b_code, b_code) == p_grp:
                            b_rv = b_data.get('rv100', 0)
                            break
                if b_rv is not None:
                    wrv += usage * b_rv
                    wu_acc += usage
            if wu_acc >= 0.40:
                wrv_per_wu = wrv / wu_acc
                wu_val = wu_acc

        wx_f = calc_weather_factor(home_team, pk, dc['weather'])
        st_f = get_spring_factor(bid, spring)
        pit_rf = get_pitcher_recent_factor(opp_id, dc['pit_stats'], dc['pitcher_recent'])
        ha_f = get_home_away_factor(bid, is_home, splits, b)
        dn_bat_f = get_day_night_batter_factor(bid, time_slot, splits, b)
        wx_hist_f = get_weather_hist_factor(bid, pk, home_team, dc['weather'],
                                            dc.get('weather_splits', {}), b)
        raw_bvp_val = get_bvp_factor(bid, opp_id, dc['bvp_career'], b)

        # Form intermediates
        rd = dc['recent_bat'].get(bid, {})
        r_pa_val = rd.get('pa', 0)
        r_hr_val = rd.get('hr', 0)
        season_rate = hr / pa if pa > 0 else 0

        base = (rate * park * platoon * hh_f * bt_f * wx_f * st_f
                * pit_rf * ha_f * dn_bat_f * wx_hist_f * AVG_PA_GAME)

        flat_base.append(base)
        flat_raw_vuln.append(raw_vuln)
        flat_brl_ratio.append(brl_ratio)
        flat_ev_ratio.append(ev_ratio)
        flat_wrv.append(wrv_per_wu)
        flat_wu.append(wu_val)
        flat_r_pa.append(r_pa_val)
        flat_r_hr.append(r_hr_val)
        flat_srate.append(season_rate)
        flat_shr.append(hr)
        flat_raw_bvp.append(raw_bvp_val)
        row_idx += 1

    # Close last game of this date
    if cur_game_pk is not None and row_idx > cur_game_start:
        bm = dc['batter_map'].get(cur_game_pk, {})
        all_bids = set(bm.get('home', [])) | set(bm.get('away', []))
        had_hr = any(b2 in dc['actual'] for b2 in all_bids)
...
arr_base     = np.array(flat_base, dtype=np.float64)
arr_vuln     = np.array(flat_raw_vuln, dtype=np.float64)
arr_brl      = np.array(flat_brl_ratio, dtype=np.float64)
arr_ev       = np.array(flat_ev_ratio, dtype=np.float64)
arr_wrv      = np.array(flat_wrv, dtype=np.float64)
arr_wu       = np.array(flat_wu, dtype=np.float64)
arr_rpa      = np.array(flat_r_pa, dtype=np.float64)
arr_rhr      = np.array(flat_r_hr, dtype=np.float64)
arr_srate    = np.array(flat_srate, dtype=np.float64)
arr_shr      = np.array(flat_shr, dtype=np.float64)
arr_bvp_raw  = np.array(flat_raw_bvp, dtype=np.float64)
...
n_rows = len(arr_base)
print(f'  {n_rows} pre-computed batter-game rows across {len(TEST_DATES)} dates')

# Build flat game arrays for np.add.reduceat
reduceat_indices = []   # start index of each game in the flat row array
game_hadhr_flat  = []   # True/False per game
game_info_flat   = []   # (pk, home, away) per game
date_game_range  = {}   # {date_str: (flat_game_start, flat_game_end)}

flat_gi = 0
for date_str in TEST_DATES:
    gi_start = flat_gi
    for (pk, start, end, had_hr, home, away) in game_slices[date_str]:
        reduceat_indices.append(start)
        game_hadhr_flat.append(had_hr)
        game_info_flat.append((pk, home, away))
        flat_gi += 1
    date_game_range[date_str] = (gi_start, flat_gi)

reduceat_idx  = np.array(reduceat_indices)
game_hadhr    = np.array(game_hadhr_flat, dtype=bool)
n_games_total = len(reduceat_idx)
print(f'  {n_games_total} total game slots')

# ── Vectorized grid search ────────────────────────────────────────────────────
import heapq

TOP_KEEP = 100   # only keep top configs by accuracy
heap = []        # min-heap of (avg_prec, ci, cfg, per_date_prec)
...
for ci, cfg in enumerate(ALL_TUNE_CONFIGS, 1):
    ...
    vuln  = np.minimum(arr_vuln, cfg['vuln_cap'])
    brl_f = arr_brl ** cfg['pwr_brl_exp']
    ev_f  = arr_ev ** cfg['pwr_ev_exp']
    pwr_f = np.clip(brl_f * ev_f, POWER_CAP[0], POWER_CAP[1])

    # Pitch matchup
    pm_f = np.ones(n_rows)
    if cfg['matchup_scale'] > 0:
        pm_f[mask_wu] = np.clip(
            np.exp(arr_wrv[mask_wu] * cfg['matchup_scale']),
            PITCH_MATCHUP_CAP[0], PITCH_MATCHUP_CAP[1])

    # Recent form
    form_f = np.ones(n_rows)
    form_mask = (arr_rpa >= cfg['form_min_pa']) & mask_shr & (arr_srate > 0)
    if form_mask.any():
        recent_rate = arr_rhr[form_mask] / arr_rpa[form_mask]
        raw_f = recent_rate / arr_srate[form_mask]
        conf_f = np.minimum(arr_rpa[form_mask], 60) / 60
        form_f[form_mask] = np.clip(
            conf_f * raw_f + (1 - conf_f),
            RECENT_FORM_CAP[0], cfg['form_cap_hi'])

    # BvP with weight
    bvp_f = 1.0 + cfg['bvp_weight'] * (arr_bvp_raw - 1.0)

    # Final HR probability per batter
    exp_hr = arr_base * vuln * pwr_f * pm_f * form_f * bvp_f * cfg['calibration']
    # log(1 - hr_prob) = log(exp(-exp_hr)) = -exp_hr
    log_1_minus_p = -exp_hr

    # Sum log(1-p) per game using reduceat → log(P(no HR per game))
    game_log_pnohr = np.add.reduceat(log_1_minus_p, reduceat_idx)
    game_pnohr     = np.exp(game_log_pnohr)

    # Score per date: pick top-N games, count correct
    n_pick = cfg['games_to_pick']
    total_acc = 0.0
    per_date_acc = {}
    for date_str in TEST_DATES:
        gs, ge = date_game_range[date_str]
        pnohr_day = game_pnohr[gs:ge]
        hadhr_day = game_hadhr[gs:ge]
        n_day = ge - gs

        k = min(n_pick, n_day)
        if k == 0:
            per_date_acc[date_str] = 0.0
            continue

        # argpartition: get indices of top-k by pnohr (descending)
        if k >= n_day:
            top_idx = np.arange(n_day)
        else:
            top_idx = np.argpartition(-pnohr_day, k)[:k]
        n_correct = int(np.sum(~hadhr_day[top_idx]))
        acc = n_correct / k
        per_date_acc[date_str] = round(acc, 3)
        total_acc += acc

    avg_acc = total_acc / len(TEST_DATES)

    # Keep top configs in a min-heap
    if len(heap) < TOP_KEEP:
        heapq.heappush(heap, (avg_acc, ci, cfg, per_date_acc))
    elif avg_acc > heap[0][0]:
        heapq.heapreplace(heap, (avg_acc, ci, cfg, per_date_acc))
    if avg_acc > best_acc:
        best_acc = avg_acc

    if ci % 50000 == 0 or ci == n_configs:
        elapsed = time.time() - t_start
        rate = ci / elapsed if elapsed > 0 else 0
        print(f'  {ci:>9}/{n_configs} configs  '
              f'{rate:,.0f}/s  best={best_acc:.1%}  '
              f'({elapsed:.0f}s elapsed)')

# Convert heap to sorted list (descending accuracy)
heap.sort(key=lambda x: -x[0])

# Re-run best config to get full date_picks for reporting
best_cfg = heap[0][2]
# Recompute hr_prob for best config
vuln  = np.minimum(arr_vuln, best_cfg['vuln_cap'])
brl_f = arr_brl ** best_cfg['pwr_brl_exp']
ev_f  = arr_ev ** best_cfg['pwr_ev_exp']
pwr_f = np.clip(brl_f * ev_f, POWER_CAP[0], POWER_CAP[1])
pm_f = np.ones(n_rows)
if best_cfg['matchup_scale'] > 0:
    pm_f[mask_wu] = np.clip(
        np.exp(arr_wrv[mask_wu] * best_cfg['matchup_scale']),
        PITCH_MATCHUP_CAP[0], PITCH_MATCHUP_CAP[1])
form_f = np.ones(n_rows)
form_mask = (arr_rpa >= best_cfg['form_min_pa']) & mask_shr & (arr_srate > 0)
if form_mask.any():
    recent_rate = arr_rhr[form_mask] / arr_rpa[form_mask]
    raw_f = recent_rate / arr_srate[form_mask]
    conf_f = np.minimum(arr_rpa[form_mask], 60) / 60
    form_f[form_mask] = np.clip(
        conf_f * raw_f + (1 - conf_f),
        RECENT_FORM_CAP[0], best_cfg['form_cap_hi'])
bvp_f = 1.0 + best_cfg['bvp_weight'] * (arr_bvp_raw - 1.0)
exp_hr = arr_base * vuln * pwr_f * pm_f * form_f * bvp_f * best_cfg['calibration']
game_log_pnohr = np.add.reduceat(-exp_hr, reduceat_idx)
game_pnohr = np.exp(game_log_pnohr)

# Build date_picks for best config
best_date_picks = {}
for date_str in TEST_DATES:
    gs, ge = date_game_range[date_str]
    day_preds = []
    for gi in range(gs, ge):
        pk, home, away = game_info_flat[gi]
        day_preds.append((float(game_pnohr[gi]), pk, bool(game_hadhr[gi]), home, away))
    day_preds.sort(key=lambda x: -x[0])
    best_date_picks[date_str] = day_preds[:best_cfg['games_to_pick']]

# Build grid_results for display
grid_results = []
for (acc, ci_val, cfg, per_date) in heap:
    grid_results.append({
        'cfg': cfg, 'avg_prec': acc,
        'per_date_prec': per_date,
        'date_picks': best_date_picks if cfg is best_cfg else {},
    })
grid_results[0]['date_picks'] = best_date_picks
...
for date_str in TEST_DATES:
    ...
    dc        = date_cache[date_str]
    # picks are tuples: (p_no_hr, pk, had_hr, home, away)
    n_correct = sum(1 for p in picks if not p[2])
...
print(f"  {'-'*58}")
for p in sorted(last_picks, key=lambda x: -x[0]):
    # p = (p_no_hr, pk, had_hr, home, away)
    matchup = f"{p[4]} @ {p[3]}"
    correct = not p[2]
    print(f"  {matchup:<34} {p[0]:>8.1%}  {'YES (0 HRs)' if correct else 'NO  (HR hit)'}")