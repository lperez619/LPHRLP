...
for date_str in TEST_DATES:
    ...
    m_gbm = HistGradientBoostingClassifier(
        max_iter=300, max_leaf_nodes=31, max_depth=5,
...
# ── Mode E: Ensemble (average of LogReg + GBM CV probs) ──────────────────────
cv_probs_e = 0.5 * cv_probs_c + 0.5 * cv_probs_d
mode_e_ll  = log_loss(val_y, cv_probs_e)
mode_e_auc = roc_auc_score(val_y, cv_probs_e) if val_y.sum() > 0 else 0.0
mode_e_full_probs = 0.5 * mode_c_full_probs + 0.5 * mode_d_full_probs
print(f'\n  [Mode E] Ensemble (LogReg+GBM average):')
