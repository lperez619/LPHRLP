...
_trainer_path = os.path.join(_SCRIPT_DIR, 'HR_Trainer_Odds.py')
...
fetch_recent_hitting = _trainer_globals['fetch_recent_hitting']
fetch_pitcher_recent = _trainer_globals['fetch_pitcher_recent']
...
if artifact is None:
    ...
    print('  Run "HR_Trainer_Odds.py" first to train the model.')
    sys.exit(1)
...
gbm_calibrator = artifact.get('gbm_calibrator')
nn_calibrator  = artifact.get('nn_calibrator')
model_nn       = artifact.get('model_nn')
scaler_nn      = artifact.get('scaler_nn')
meta_model_f   = artifact.get('meta_model')
model_hist     = artifact.get('model_hist')
scaler_hist    = artifact.get('scaler_hist')
...
load_day_odds = _trainer_globals['load_day_odds']
day_odds_today = load_day_odds(TARGET_DATE)
if day_odds_today:
    print(f'  Implied Odds Loaded: {len(day_odds_today)} players for today')
else:
    print(f'  Implied Odds: No odds found cached for {TARGET_DATE}')

savant_cache = load_cache('savant_rolling_cache') or {}
savant_today = savant_cache.get(TARGET_DATE, {})
if not savant_today and savant_cache:
    savant_today = savant_cache[max(savant_cache.keys())]

tpc = load_cache('team_pitching_cache') or {}
t_year = tpc.get(int(TARGET_DATE[:4]), {})
team_pitch_today = t_year.get(TARGET_DATE, {})
if not team_pitch_today and t_year:
    team_pitch_today = t_year[max(t_year.keys())]

print(f'  Savant rolling cache: extracted {len(savant_today)} batters')
print(f'  Team pitching cache: extracted {len(team_pitch_today)} teams')
...
recent_bat = fetch_recent_hitting(all_bat, TARGET_DATE)
pitcher_recent = fetch_pitcher_recent(all_pit, TARGET_DATE)
...
for g in games:
    ...
    for side, opp_id, is_home in [
        ...
        for lineup_idx, bid in enumerate(lineup):
            ...
            # Fetch day_odds if available
            player_odds = 0.05
            if day_odds_today:
                # Need player name for odds lookup - use raw data
                p_name = None
                if bid in raw_25: p_name = raw_25[bid].get('name')
                elif bid in raw_26: p_name = raw_26[bid].get('name')
                
                if p_name:
                    p_name_lower = p_name.lower().replace("'", "").replace(".", "")
                    if p_name_lower in day_odds_today:
                        player_odds = day_odds_today[p_name_lower]
            ...
            row = _precompute_row(
                ...
                savant_day=savant_today, team_pitch=team_pitch_today,
                batter_zones=batter_zones_global, pitcher_zones=pitcher_zones_global,
                day_odds=player_odds
            )
            if row is not None:
...
# Compute hist_prior from historical model
if model_hist is not None and scaler_hist is not None:
    _hp = model_hist.predict_proba(scaler_hist.transform(core_X))[:, 1]
    hist_prior_col = np.log(np.clip(_hp, 1e-8, 1 - 1e-8) /
                            np.clip(1 - _hp, 1e-8, 1)).reshape(-1, 1).astype(np.float32)
    print(f'  hist_prior: min={hist_prior_col.min():.3f} max={hist_prior_col.max():.3f}')
else:
    hist_prior_col = np.zeros((len(rows), 1), dtype=np.float32)
    print('  hist_prior: zeros (no historical model in artifact)')

all_X = np.hstack([core_X, adj_X, hist_prior_col])
...
if best_mode == 'F' and meta_model_f is not None and model_gbm is not None and model_nn is not None:
    # Meta-learner: LogReg stacking on [LR, GBM, NN]
    probs_lr = model_logreg.predict_proba(scaler.transform(all_X))[:, 1]
    probs_gbm = model_gbm.predict_proba(all_X)[:, 1]
    probs_nn = model_nn.predict_proba(scaler_nn.transform(all_X))[:, 1]
    if gbm_calibrator is not None:
        probs_gbm = gbm_calibrator.predict(probs_gbm)
    if nn_calibrator is not None:
        probs_nn = nn_calibrator.predict(probs_nn)
    meta_X = np.column_stack([probs_lr, probs_gbm, probs_nn])
    probs = meta_model_f.predict_proba(meta_X)[:, 1]
elif best_mode == 'E' and model_gbm is not None:
    # Ensemble: weighted LogReg + calibrated GBM
    probs_lr = model_logreg.predict_proba(scaler.transform(all_X))[:, 1]
    probs_gbm = model_gbm.predict_proba(all_X)[:, 1]
    if gbm_calibrator is not None:
        probs_gbm = gbm_calibrator.predict(probs_gbm)
...
elif best_mode == 'NN' and model_nn is not None:
    probs = model_nn.predict_proba(scaler_nn.transform(all_X))[:, 1]
    if nn_calibrator is not None:
        probs = nn_calibrator.predict(probs)
elif best_mode == 'A':
...
# Break probability ties using raw power (core feature index 4: log_power)
_pwr = core_X[:, 4]
probs = probs + 1e-8 * (_pwr - _pwr.min())
