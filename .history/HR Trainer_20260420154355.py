...
if train_X is not None and train_y is not None and model_a is not None and scaler_a is not None:
    n_adj_cols = val_adj_X.shape[1]          # 9
    # Impute adj features with 2026 column means (not zeros) so the model
    # doesn't learn to ignore adj features just because historical rows lack them.
    adj_means  = val_adj_X.mean(axis=0)      # shape (9,)
    hist_pad   = np.tile(
        np.append(adj_means, 0.0).astype(np.float32),
        (len(train_y), 1)
    )                                         # shape (n_hist, 10) — last col = hist_prior
...
print(f'  Top-10 precision (train): C={prec_c:.1%}  D={prec_d:.1%}  E={prec_e:.1%}  F={prec_f:.1%}  G={prec_g:.1%}  H={prec_h:.1%}  A={prec_a:.1%}')
...
print(f'  Top-10 precision (holdout): C={_hp_c:.1%}  D={_hp_d:.1%}  E={_hp_e:.1%}  F={_hp_f:.1%}  G={_hp_g:.1%}  H={_hp_h:.1%}  A={_hp_a:.1%}')
